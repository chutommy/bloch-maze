...
class Render():
    ...
    def __draw_rect(self, rect, color, text=None, text_color=WHITE):
        pygame.draw.rect(self.screen, color, rect)
        if text is not None:
            self.__draw_text(text, text_color, rect.center)

    def __draw_cell(self, maze, x, y, rect, start_state, end_state):
        pygame.draw.rect(self.screen, CELL_COLORS[maze.grid[y, x]], rect)

        if maze.grid[y, x] == Cell.ENTRANCE:
            self.__draw_text(start_state, WHITE, rect.center)
        elif maze.grid[y, x] == Cell.EXIT:
            self.__draw_text(end_state, WHITE, rect.center)
        elif maze.grid[y, x] not in [Cell.NONE, Cell.WALL]:
            self.__draw_text(maze.grid[y, x], WHITE, rect.center)

    def render_cell(self, maze, x, y, start_state, end_state):
        rect = pygame.Rect(get_rect(x, y, self.cell_size))
        self.__draw_cell(maze, x, y, rect, start_state, end_state)

    def __render_maze(self, maze, start_state, end_state):
    ...
    def render_player(self, maze, player, current_state):
        ...
        if maze[player] == Cell.NONE:
            self.__draw_rect(rect, CELL_COLORS[Cell.NONE], current_state, CURRENT_STATE_COLOR)
        pygame.draw.rect(self.screen, BLACK, rect, 3)
        return rect

    def render_game(self, game, start_state, end_state, current_state):
        self.__render_maze(game.maze, start_state, end_state)
